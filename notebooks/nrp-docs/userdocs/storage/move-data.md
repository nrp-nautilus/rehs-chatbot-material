...
- Run an idle pod , kubectl exec into it and manually run a command. You’ll have to set up the credentials to access the remote storage.

- Or, run a batch job which will do this for you. In this case the pod should have credentials set up at the time it starts. This should be better for large datasets, since you don’t have to keep your shell open, and will auto restart if the pod is killed for some reason.
...
For moving data directly between two PersistentVolumeClaims (PVCs), you can also use tools such as pv-migrate . pv-migrate creates temporary Kubernetes resources to copy data from one PVC to another, which can be useful when you need to migrate volumes without manually creating a copy pod.
