...
Ephemeral storage on Nautilus: Pods that write more than 50Gi of ephemeral scratch data per container (for example, to an emptyDir at /mnt/data ) can be evicted. When your code uses emptyDir or other scratch space, set resources.requests.ephemeral-storage in the container (and optionally resources.limits.ephemeral-storage ) to the largest scratch size you expect (as in the example below). See Local Scratch for more details and complete YAML.
